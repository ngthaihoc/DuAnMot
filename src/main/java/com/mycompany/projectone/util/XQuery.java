...
public class XQuery {
   ...
   private static <B> B readBean(ResultSet resultSet, Class<B> beanClass) throws Exception {
    ...
    // Debug: In danh sách cột trong ResultSet
    java.sql.ResultSetMetaData metaData = resultSet.getMetaData();
    int columnCount = metaData.getColumnCount();
    System.out.println("Cac cot trong ResultSet:");
    for (int i = 1; i <= columnCount; i++) {
        System.out.println("Cot " + i + ": " + metaData.getColumnName(i) + " (Label: " + metaData.getColumnLabel(i) + ")");
    }
    ...
    for (Method method : methods) {
        ...
        if (name.startsWith("set") && method.getParameterCount() == 1) {
            try {
                ...
                if (value instanceof BigDecimal && method.getParameterTypes()[0] == double.class) {
